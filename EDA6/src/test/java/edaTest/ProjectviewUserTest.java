...
public class ProjectviewUserTest extends Base {
	
	public static Logger log =LogManager.getLogger(ProjectviewUserTest.class.getName());
	@BeforeMethod
	...
	public void UserActiveInacive() throws IOException, InterruptedException
	{
		...
		Thread.sleep(2000);
		wp.getsignIn().click();*/
		log.info("User acive and inactive for projects user");
		...
		Thread.sleep(5000);
		Pp.getViewUser().click();
		log.info("Enter project user page");
	...
	public void EditUerforDepartment() throws IOException, InterruptedException
	{
		...
		Thread.sleep(2000);
		wp.getsignIn().click();*/
		log.info("Edit user for Project user");
		LoginPage lip=new LoginPage(driver);
		lip.getUsername().clear();
		lip.getUsername().sendKeys(prop.getProperty("username"));
		lip.getPassword().clear();
		...
		for(int i=1;i<ActiveInSize;i++) {
			...
			if(dVp.getEdit_user().isDisplayed()) {
				...
				uEp.getName().clear();
				uEp.getName().sendKeys("samarpita");
				Thread.sleep(1000);
				uEp.getMiddleName().sendKeys("saha");
				Thread.sleep(1000);
				uEp.getLastName().sendKeys("basu");
				Thread.sleep(1000);
